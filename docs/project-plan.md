...
| T27 | Model idle-unload (in-process): `myna-server --sleep-idle-seconds`, `adapter.unload()` releasing model weights + GPU memory, idle monitor in the server | done | Claude/Charles | T14a | **Done:** `--sleep-idle-seconds N --idle-action unload`; `unload()` on both adapters (drop ref + gc; NeMo also `torch.cuda.empty_cache()`); `LifecycleService` + `idle_monitor` (`myna/server/lifecycle.py`). Frees the **weights** (the bulk); process + CUDA context stay (full release is T28). Re-warm is faster than cold (context resident). `progress.phase=preparing` covers the re-warm UX. Verify via `nvidia-smi` |
| T28 | snapd socket activation: snapd owns the UDS, daemon starts on first connect and **exits after idle**, relaunched on next connection — full process/VRAM release at idle | in progress | Claude/Charles | T14b, T27, T26 | **Server mechanism done:** `serve_unix` serves on an inherited fd via `systemd_socket()` (`LISTEN_FDS`/`LISTEN_PID`); `--idle-action exit`; verified serving on a pre-bound socket. Dev-test without the snap: `systemd-socket-activate -l /tmp/s.sock -- .venv/bin/myna-server …` (use the console script, **not** `uv run` — a wrapper breaks `LISTEN_PID`). **Remaining:** the `sockets:` stanza in snapcraft.yaml (snap packaging). Trades zero idle cost for a cold re-init per wake (heavy for NeMo/torch) → leans on T26's loading signal; interacts with T14c (a connection triggers a wake) |
