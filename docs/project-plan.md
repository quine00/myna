...
| T27 | Model idle-unload (in-process): `myna-server --sleep-idle-seconds`, `adapter.unload()` releasing model weights + GPU memory, idle monitor in the server | todo | | T14a | Surfaced 2026-06-14: GPU server stays resident after dictation. Frees model **weights** (the bulk) after idle; the CUDA context (~100–300 MB) and runtime stay until process exit. Testable via `nvidia-smi`. Next utterance pays a re-warm (faster than full cold — context stays). Complementary to T28 |
| T28 | snapd socket activation: snapd owns the UDS, daemon starts on first connect and **exits after idle**, relaunched on next connection — full process/VRAM release at idle | todo | | T14b, T27, T26 | The design note's lifecycle plan. Requires `serve_unix` to accept a passed-in fd (`sd_listen_fds`/`LISTEN_FDS`), an idle-exit timer, and a `sockets:` stanza in snapcraft.yaml. Dev-testable without the snap via `systemd-socket-activate`. Trades zero idle cost for a cold CUDA re-init on every wake (worse for NeMo/torch than CTranslate2) → raises the stakes on T26 (a "loading model…" signal). Interacts with T14c (who may connect, and thus trigger a wake) |
